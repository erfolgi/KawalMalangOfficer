...
class PatrolUseCase(private val repository: AppRepository) {
    ...
    suspend fun execDoPatrol(token: String, body: ArrayList<String>): Resource<BaseResponse<PatrolData>> =
        repository.doPatrol(token, body)
    suspend fun execFinishPatrol(token: String): Resource<BaseResponse<PatrolData>> =
        repository.finishPatrol(token)
    suspend fun execGetPatrolHistory(token: String): Resource<ArrayList<PatrolData>> =
        repository.getPatrolHistory(token)
